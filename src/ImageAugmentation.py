...
class ImageAugmentation:
    ...
    def __init__(self,
        ...
        shear_x : float
            intensity of shear along x axis (@see tfa.image.shear_x())
        shear_y : float
            intensity of shear along y axis (@see tfa.image.shear_y())
    ...
    def __call__(self, dataset):
        ...
        def rand_range(lower, upper):
            ...
            return tf.random.uniform(shape=[], minval=lower, maxval=upper, dtype=self.dtype)


        # Random shear-x
        if self.shear_x != 0:
            dataset = dataset.map(lambda x, y: 
                ( tf.cast(tfa.image.shear_y(tf.cast(x, 'uint8'), self.shear_x, 0), self.dtype), y )
            )

        # Random shear-y
        if self.shear_y != 0:
            dataset = dataset.map(lambda x, y: 
                ( tf.cast(tfa.image.shear_y(tf.cast(x, 'uint8'), self.shear_y, 0), self.dtype), y )
            )

        # Random rotations
        if self.rotation_range[0] != 0 or self.rotation_range[1] != 0:
            dataset = dataset.map(lambda x, y: 
                ( tfa.image.rotate(x, rand_range(self.rotation_range[0], self.rotation_range[1])), y )
            )
